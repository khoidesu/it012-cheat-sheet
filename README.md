...
> Quy ước biến: giả sử `a` là mảng int đã có địa chỉ gốc nằm trong `$s0`, `i` nằm trong `$s1`, `sum` nằm trong `$s2` (mỗi phần tử `int` chiếm 4 byte).
...
## 11. Nhập / Xuất (Input / Output)

### 11.1 Nhập – xuất **một số nguyên**
...
> Ghi chú: `bge` (branch if greater or equal) là pseudo-instruction, tương đương `slt` + `beq`. Có thể thay bằng:
> ```asm
> slt  $t3, $s2, $s1
...
- **`$t` (temporary)**: không được bảo toàn qua lời gọi hàm — hàm được gọi có thể ghi đè.
- **`$s` (saved)**: phải được bảo toàn qua lời gọi hàm — nếu hàm dùng thì phải lưu/khôi phục trên stack.
- **Offset mảng**: với kiểu `int` (4 byte) dùng `sll ..., 2` (nhân 4); với `char` (1 byte) không cần dịch bit; với `double` (8 byte) dùng `sll ..., 3` (nhân 8).
- **`li`, `la`, `move`, `neg`, `slti`** là các *pseudo-instruction* — trình hợp dịch (assembler) sẽ tự mở rộng thành lệnh máy thật (ví dụ `li` → `addi` hoặc `lui`+`ori`).
